...
sub readString
{
    ...
    #Arshi Dec7'2010, adding variables to accept as passed in Observable.pm readString
    my $AllowNewStates       = shift;
    my $AllowUndefinedStates = shift;
	...
	if ($mtlist)
	{
		$err = $mtlist->checkSpeciesGraph( $sg, { IsSpecies            => $is_species,
                                                  ...
                                                  AllowNewStates       => $AllowNewStates,
                                                  AllowUndefinedStates => $AllowUndefinedStates } );
		if ($err) {  return ($err);  }
	}
...
sub checkSpecies
{
    ...
    my $params = { IsSpecies            => 1,
                   AllowNewTypes        => 0,
                   AllowNewStates       => 0,
                   AllowUndefinedStates => 0  };
    
    # check that components are fully specieied
