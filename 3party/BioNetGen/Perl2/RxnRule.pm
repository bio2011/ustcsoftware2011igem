...
sub newRxnRule
{
	...
	# Bug fixed here --Justin
	# used to skip over species to find the first ":"
	# Now we restrict rule labels to a string of word characters, whitespace
	# and "*" followed by a colon.
	if ( $string =~ s/^([\w\s*]+):\s*// )
	{
	    # Check if first token is a name for the reaction (first occurence of a ':')
		$name = $1;
		#print "(user) name=$name\n";
	}

	# Check if token is an index !!! don't allow this anymore! --Justin !!!
	elsif ( $string =~ s/^(\d+)\s+// )
	{
		$name = $1;
			...
			foreach my $rxn_label (@$rxn_labels)
			{
                ...
                if ( %{$rr->RRefs}  and  $rr->RateLaw->Type eq 'Function' )
                {
                    ...
                    if ( $fcn->checkLocalDependency($plist) )
                    {
                        ...
                        # get local function
                        (my $local_fcn, $err ) = $fcn->evaluate_local( \@local_refs, $plist );
